...
const TopRated = () => {
      ...
      try {
        ...
        const processedPlaces = fetchedPlaces.map((place) => {
          const placeReviews = allReviews.filter(
            (review) => review.placeid == place.placeid
          );
          console.log(placeReviews); // Filter reviews for this place
          const averageRating =
            placeReviews.length > 0
              ? placeReviews.reduce((acc, review) => acc + review.rating, 0) /
                placeReviews.length
              : 0;
        ...
        const sortedTop3 = top3Places.sort(
          (a, b) => b.averageRating - a.averageRating
        ); // Sort the top 3 by averageRating
  ...
  return (
    <section className="top-rated-section px-4 py-8 md:px-8 lg:px-16">
      {" "}
      {/* Added section and padding */}
      <h2 className="text-2xl font-bold mb-4 text-primaryLight">
        Top Rated Places
      </h2>{" "}
      {/* Heading with margin */}
      <p className="text-xl mb-4 text-primaryLight">
        Explore the top places rated by our users
      </p>
